    ...
    if (m_expr != nullptr)
    {
        ...
        if (!returnType->IsPlaceholderType())
        {
            if (!returnType->IsAnyType() && !returnType->TypeEqual(*m_exprType))
            {
                ...
                {
                    // insert cast
                    m_overrideExpr = MakeHandle<AstAsExpression>(
                        CloneAstNode(m_expr),
                        MakeHandle<AstTypeSpecifier>(
                            MakeHandle<AstTypeRef>(returnType, m_location),
                            m_location),
                        m_location);
...
UniquePtr<Buildable> AstReturnStatement::Build(AstVisitor* visitor, Module* mod)
{
    ...
    const Handle<AstExpression>& expr = m_overrideExpr ? m_overrideExpr : m_expr;
...
Handle<AstStatement> AstReturnStatement::Clone() const
{
    return CloneImpl();
}
